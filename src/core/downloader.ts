...
class Downloader extends EventEmitter {
    ...
    async download() {
        ...
        const { title, mpdUrl, isLowLatencyLiveStream, isPremiumVideo, videoId } = await YouTubeService.getVideoInfo(
            this.videoUrl
        );
        if (!mpdUrl) {
        ...
        this.outputFilename = escapeFilename(`[${videoId}]${title}_ts0.mp4`);
